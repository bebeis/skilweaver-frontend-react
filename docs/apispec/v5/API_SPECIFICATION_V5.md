...
> [!IMPORTANT]
> 이 문서는 v5.0 API 명세서입니다. 모든 API를 포함하며, 학습 플랜에서 학습 목표로의 전환 기능이 추가되었습니다.
...
### 3.5. 학습 스트릭 조회 (V5 Phase 4)

> [!TIP]
> 학습 연속일(스트릭) 정보를 조회합니다. 동기부여를 위한 게이미피케이션 요소입니다.
...
```json
{
  ...
  "data": {
    ...
    "progressChange": 15,
    "milestones": [
      {"milestone": "첫 스텝 완료", "achievedAt": "2025-12-10T10:00:00", "achieved": true},
      {"milestone": "목표 25% 달성", "achievedAt": null, "achieved": false}
    ],
...
```json
{
  ...
  "data": {
    ...
    "learningDays": 15,
    "averageDailyHours": 1.3,
    "longestStreakInMonth": 7,
    "progressChange": 45,
    "milestones": [
      {"milestone": "목표 50% 달성", "achievedAt": "2025-12-11T14:00:00", "achieved": true},
      {"milestone": "7일 연속 학습", "achievedAt": "2025-12-08T18:00:00", "achieved": true}
    ],
    "weeklyBreakdown": [
      {"weekNumber": 1, "completedSteps": 4, "learningHours": 8, "learningDays": 5}
    ]
  }
}
```
...
> [!TIP]
> 이 API는 V5에서 신규 추가되었습니다. 학습 플랜을 시작하고 연결된 학습 목표를 자동으로 생성합니다.
...
```json
{
  "goalTitle": "Kotlin Coroutines 마스터하기",
  "goalDescription": "4주 완성 코루틴 학습",
  "dueDate": "2025-12-31",
  "priority": "HIGH"
}
```
...
|-------|------|----------|-------------|
| goalTitle | String | No | 목표 제목 (미제공 시 "{기술명} 학습하기" 자동 생성) |
| goalDescription | String | No | 목표 설명 (미제공 시 자동 생성) |
| dueDate | LocalDate | No | 목표 기한 |
| priority | GoalPriority | No | 우선순위 (default: MEDIUM) |
...
```json
{
  ...
  "data": {
    "learningGoalId": 5,
    "learningPlanId": 1,
    "title": "Kotlin Coroutines 마스터하기",
    "description": "AI 추천 학습 플랜을 통해 Kotlin Coroutines을(를) 학습합니다.",
    "dueDate": "2025-12-31",
    "priority": "HIGH",
    "status": "IN_PROGRESS",
    "totalSteps": 7,
    ...
    "createdAt": "2025-12-12T03:00:00",
    "updatedAt": "2025-12-12T03:00:00"
  },
...
**Business Logic**
1. 학습 플랜 상태를 ACTIVE로 변경 (이미 ACTIVE면 유지)
2. `startedAt` 필드를 현재 시각으로 설정 (최초 시작 시)
3. 학습 목표 자동 생성:
   - `status`: IN_PROGRESS
   - `learningPlanId`: FK로 플랜과 연결
   - `totalSteps`: 플랜 전체 스텝 개수
   - `completedSteps`: 0
   - `progressPercentage`: 0
4. 응답에 생성된 Goal과 연결된 Plan 정보 포함

**Error Responses**
- `404`: LEARNING_PLAN_NOT_FOUND - 플랜이 없거나 접근 권한 없음
