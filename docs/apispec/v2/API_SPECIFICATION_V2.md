...
> [!IMPORTANT]
> 이 문서는 v2.0 API 명세서입니다. v1.3의 모든 API를 포함하며, Phase 4 피드백 API가 추가되었습니다.
> v2.0 신규/변경사항은 `v2_changes.md` 파일을 참조하세요.
...
기본 필드:
- `technologyId`, `key`, `displayName`, `category`, `ecosystem`, `officialSite`, `active`

지식 베이스:
- `knowledge`: TechnologyKnowledge 정보
- `prerequisites`: 선행 지식 목록 (TechnologyKnowledge 기반)
- `useCases`: 사용 사례 목록

**v2 신규 - 학습 메타데이터 (Phase 3):**
- `learningRoadmap` (String?): 학습 로드맵 설명
- `estimatedLearningHours` (Int?): 예상 학습 시간
- `relatedTechnologies` (List<String>): 관련 기술 key 목록
...
```json
{
  ...
  "learningRoadmap": "1단계: 기본 문법 2단계: 객체지향 3단계: 함수형 프로그래밍 4단계: Coroutines",
...
**선택 필드:**
- `ecosystem`, `officialSite`
- **v2 학습 메타데이터**: `learningRoadmap`, `estimatedLearningHours`, `prerequisites`, `relatedTechnologies`, `communityPopularity`, `jobMarketDemand`
...
**선택 필드 (모두 optional):**
- 기본: `displayName`, `ecosystem`, `officialSite`, `active`
- **v2 학습 메타데이터**: `learningRoadmap`, `estimatedLearningHours`, `prerequisites`, `relatedTechnologies`, `communityPopularity`, `jobMarketDemand`
