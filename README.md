...
**현황**: 활발한 개발 중 | **버전**: 0.1.0 | **최종 업데이트**: 2025년 11월

## 🚀 기술 스택

- **프레임워크**: React 18.2.0 + TypeScript 5.3.3
- **빌드 도구**: Vite 5.0.8
- **라우팅**: React Router v6.20.0
- **스타일링**: TailwindCSS 3.4.0 + shadcn/ui
- **UI 라이브러리**: Radix UI (25+ 컴포넌트)
- **상태 관리**: React Context API + React Hook Form 7.66.1
- **차트**: Recharts 3.5.0
- **테마**: next-themes 0.4.6
- **테스팅**: Vitest 1.0.4 + Testing Library 14.1.2 + MSW 2.0.11
- **배포**: Vercel
...
│   ├── ui/                 # shadcn/ui 컴포넌트
│   ├── layout/             # 레이아웃 컴포넌트
│   ├── learning-plans/     # 학습 플랜 관련 컴포넌트
│   └── figma/              # Figma 연동 컴포넌트
├── pages/                  # 페이지 컴포넌트
│   ├── auth/              # 인증 페이지
│   ├── goals/             # 학습 목표 페이지
│   ├── skills/            # 기술 스택 페이지
│   ├── learning-plans/    # 학습 플랜 페이지
│   ├── settings/          # 설정 페이지
│   └── technologies/      # 기술 카탈로그 페이지
├── src/
│   ├── lib/
│   │   └── api/           # API 클라이언트 & 타입 정의
│   │       ├── auth.ts
...
- ✅ **사용자 인증**: 회원가입, 로그인, 토큰 기반 인증
- ✅ **기술 스택 관리**: 사용자의 기술 스택 추가/수정/삭제
- ✅ **학습 목표 설정**: 학습 목표 생성 및 관리
- ✅ **AI 기반 학습 플랜 생성**: AI를 활용한 맞춤형 학습 계획 자동 생성
- ✅ **학습 플랜 상세 조회**: 단계별 학습 계획 조회 및 진행도 추적
- ✅ **Agent Run 관리**: 학습 계획 생성 이력 관리
- ✅ **기술 카탈로그**: 전체 기술 스택 조회 및 검색
- ✅ **설정 페이지**: 사용자 프로필 및 환경설정 관리
...
```typescript
// 주요 API 모듈들
- src/lib/api/auth.ts          # 인증 관련 (회원가입, 로그인)
- src/lib/api/skills.ts        # 기술 스택 관리 API
- src/lib/api/goals.ts         # 학습 목표 관리 API
- src/lib/api/learning-plans.ts # 학습 플랜 생성 및 조회 API
- src/lib/api/technologies.ts  # 기술 카탈로그 API
- src/lib/api/agent-runs.ts    # 에이전트 실행 이력 API
- src/lib/api/client.ts        # API 클라이언트 (fetch 기반)
- src/lib/api/types.ts         # 공통 타입 정의
```
